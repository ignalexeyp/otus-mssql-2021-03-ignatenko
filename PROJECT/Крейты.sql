...
/****************Пользователи персональные данные**********************************************/
      CREATE TABLE UsersPerson
(
    ...
    ValidTo datetime2 GENERATED ALWAYS AS ROW END NOT NULL,
PERIOD FOR SYSTEM_TIME (ValidFrom, ValidTo),
CONSTRAINT PK_UsersPerson PRIMARY KEY CLUSTERED (UsersId)
)
WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = dbo.UsersPerson_Archive)
);
GO

CREATE INDEX FI_UsersPerson_UserId ON UsersPerson(UsersId);
GO

ALTER TABLE UsersPerson ADD CONSTRAINT FK_UsersPerson_Users_UsersId FOREIGN KEY (UsersId)  REFERENCES Users (UsersId);
GO
...
CREATE UNIQUE INDEX UI_Request_UniqueId  ON Request(UniqueId) WHERE (UniqueId IS NOT NULL);
GO
...
DROP INDEX UI_OffersName ON  Offers;


ALTER TABLE offers
DROP CONSTRAINT FK_Offers_ExposureChannels;

ALTER TABLE offers
DROP COLUMN ExposureChannelsId;
...
(
  ...
  Frommail  nvarchar(40)  NOT NULL,    --- pglinka@radius-etl.ru
  Mailpass  nvarchar(40)  NOT NULL,    --- pass123
...
/*********************Статистика для кубов****************************************************************************/
create table CampaignStatisticsDW
(
   CampaignId int NOT NULL,  --- fk campaign
   ExposureChannelsId  int NOT NULL,  --- fk exposure_channels -  каналы воздействия
   OffersId  bigint NOT NULL,  --- fk offers id предложения
   ...
   QuantitySuccessful int NULL,  --- количество успешных воздействия (зафиксирована реакция)
   CampaignResultAchieved int NULL, --- количество воздействий достигнувших результата (цели кампании)
CONSTRAINT PK_CampaignStatisticsDW PRIMARY KEY CLUSTERED (CampaignId, ExposureChannelsId, OffersId)
);
GO


ALTER TABLE CampaignStatisticsDW ADD CONSTRAINT FK_CampaignStatisticsDW_Campaign
    FOREIGN KEY (CampaignId) REFERENCES Campaign (CampaignId);
GO

CREATE INDEX FI_CampaignStatisticsDW_Campaign ON CampaignStatisticsDW (CampaignId);
GO

ALTER TABLE CampaignStatisticsDW ADD CONSTRAINT FK_CampaignStatisticsDW_ExposureChannels
    FOREIGN KEY (ExposureChannelsId)  REFERENCES ExposureChannels (ExposureChannelsId);
GO

CREATE INDEX FI_CampaignStatisticsDW_ExposureChannels ON CampaignStatisticsDW (ExposureChannelsId);
GO

ALTER TABLE CampaignStatisticsDW ADD CONSTRAINT FK_CampaignStatisticsDW_Offers
    FOREIGN KEY (OffersId)
    REFERENCES Offers (OffersId); 
GO

CREATE INDEX FI_CampaignStatisticsDW_Offers ON CampaignStatisticsDW (OffersId);
GO

SELECT * FROM CampaignStatisticsDW;
...
/*********
Создание полнотекстового индекса для таблицы ExposurePerformed - таблица проведенных воздействий и отзывов.
Индекс создается по полю ResponseText nvarchar(max) --- текст ответа для поиска по словам (фразам)
*********/

CREATE FULLTEXT CATALOG Campaign_FT_Catalog
WITH ACCENT_SENSITIVITY = ON
AS DEFAULT
AUTHORIZATION [dbo];
GO

CREATE FULLTEXT INDEX ON ExposurePerformed(ResponseText LANGUAGE Russian)
KEY INDEX PK_ExposurePerformed
ON (Campaign_FT_Catalog)
WITH (
...
CREATE TABLE CampaignDW
(
    CampaignId  int NOT NULL,      --- id кампании
    CampaignName nvarchar(100) NOT NULL, --- наименование кампании
    CompanyPurpose nvarchar(500) NULL, --- цель кампании
    CampaignStatus smallint NULL, --- состояние кампании 1-ожидание, 2-выполняется, 3-выполнена, 4-остановлено выполнение  ВОЗМОЖНО ЕЩЁ КАКОЙ ТО
    DateFrom datetime2 NULL,  --- даты действия кампании
    DateTo   datetime2 NULL,
    UsersId int NULL,  --- ответственный за кампанию id пользователя 
CONSTRAINT PK_CampaignDW PRIMARY KEY CLUSTERED (CampaignId)
);
GO
...
CREATE TABLE ExposureChannelsDW  
(
    ExposureChannelsId int  NOT NULL,          --- id сети
    UniqueId int NOT NULL,     --- уникальный идентификатор канала воздействия, определяет номер разработанного интерфеса
    ExposureChannelsName nvarchar(100) NOT NULL,     --- наименование канала воздействия
    ExposureChannelswebsite  nvarchar(100)  NULL,    --- сайт сети
    Color bigint NULL, --- цвет воздействия в приложении
CONSTRAINT PK_ExposureChannelsDW PRIMARY KEY CLUSTERED (ExposureChannelsId)
);
GO


CREATE TABLE OffersDW
(
    OffersId bigint NOT NULL,        --- id предложения
    OffersName nvarchar(100) NOT NULL,   --- наименование предложения
    Heading nvarchar(250) NULL,    --- заголовок  
    File_path  nvarchar(250) NULL, --- путь к файлу  
    Offers_text nvarchar(max) NULL,  --- text NULL,  текст предложения
    File_content bit NULL,    --- загруженный файл
    Offers_type smallint NULL, --- тип (вид)  предложения 1-txt, 2-html, 3-почтовое отправление
CONSTRAINT PK_OffersDW PRIMARY KEY CLUSTERED (OffersId),
);
GO
...
ALTER TABLE CampaignStatisticsDW ADD CONSTRAINT FK_CampaignStatisticsDW_CampaignDW
    FOREIGN KEY (CampaignId) REFERENCES CampaignDW (CampaignId);
GO

CREATE INDEX FI_CampaignStatisticsDW_Campaign ON CampaignStatisticsDW (CampaignId);
GO

ALTER TABLE CampaignStatisticsDW ADD CONSTRAINT FK_CampaignStatisticsDW_ExposureChannelsDW
    FOREIGN KEY (ExposureChannelsId)  REFERENCES ExposureChannelsDW (ExposureChannelsId);
GO

CREATE INDEX FI_CampaignStatisticsDW_ExposureChannels ON CampaignStatisticsDW (ExposureChannelsId);
GO

ALTER TABLE CampaignStatisticsDW ADD CONSTRAINT FK_CampaignStatisticsDW_OffersDW
    FOREIGN KEY (OffersId)
    REFERENCES OffersDW (OffersId); 
GO
